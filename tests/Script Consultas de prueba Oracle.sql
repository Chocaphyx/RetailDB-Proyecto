...
GROUP BY e.nombre, EXTRACT(MONTH FROM v.fecha_venta)
ORDER BY mes;
...
SELECT p.nombre, 
       SUM(dv.cantidad) AS total_vendido, 
       (p.stock_minimo - SUM(dv.cantidad)) AS proyeccion_restock
FROM detalle_venta dv
JOIN productos p ON dv.id_producto = p.id_producto
...
WHERE m.fecha IS NULL OR m.fecha < SYSDATE - INTERVAL '90' DAY;
